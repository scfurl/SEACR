...
then
	...
	Rscript SEACR_1.1.R --exp=$password.auc --ctrl=$password2.auc --norm=yes --output=$password
elif [[ -f $2 ]]
then
	echo "Calculating threshold using non-normalized control: $(date)"
	Rscript SEACR_1.1.R --exp=$password.auc --ctrl=$password2.auc --norm=no --output=$password
else
	echo "Using user-provided threshold: $(date)"
	Rscript SEACR_1.1.R --exp=$password.auc --ctrl=$2 --norm=no --output=$password
fi
